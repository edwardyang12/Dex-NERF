...
def main():
    ...
    for i in trange(start_iter, cfg.experiment.train_iters):
        ...
        if USE_CACHED_DATASET:
            ...
            target_ray_values = 0.299*target_ray_values[...,0] + 0.587*target_ray_values[...,1] + 0.114*target_ray_values[...,2] 
        ...
        if (
            ...
            with torch.no_grad():
                ...
                writer.add_image(
                    "validation/rgb_coarse", cast_to_image(rgb_coarse), i
                )
                if rgb_fine is not None:
                    writer.add_image(
                        "validation/rgb_fine", cast_to_image(rgb_fine), i
                    )
...
def cast_to_image(tensor):
    ...
    if len(tensor.shape)>2:
        ...
        tensor = torch.unsqueeze(tensor, dim=0).repeat(3,1,1) # (H,W) to (3, H, W) grayscale
